# "Added persistent FAISS index per YouTube video: load existing index if available, else create and save new one." 
...
# Load VecStore if exists, else create new
if os.path.exists(index_path):
    print(f"Loading existing FAISS index for video {video_id}")
    vectorstore = FAISS.load_local(index_path, embeddings_model, allow_dangerous_deserialization=True)
else:
    print(f"Creating new FAISS index for video {video_id}")
    # SPLITER
...
result = main_chain.invoke("What is functional Interface?")
print(result)
